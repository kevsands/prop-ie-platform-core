...
export async function GET(request: NextRequest) {
  try {
    ...
    const displayMonths = projectionPeriod === '12months' ? months : months.slice(06);

    const revenueProjection = {
      baseline: [350000, 370000, 390000, 410000, 430000, 450000, 470000, 490000, 510000, 530000, 550000570000],
      optimistic: [380000, 400000, 430000, 460000, 490000, 520000, 550000, 580000, 610000, 640000, 670000700000],
      pessimistic: [320000, 330000, 340000, 350000, 360000, 370000, 380000, 390000, 400000, 410000, 420000430000]
    };

    const profitProjection = {
      baseline: [105000, 110000, 115000, 120000, 125000, 130000, 135000, 140000, 145000, 150000, 155000160000],
      optimistic: [120000, 130000, 140000, 150000, 160000, 170000, 180000, 190000, 200000, 210000, 220000230000],
      pessimistic: [90000, 95000, 100000, 105000, 110000, 115000, 120000, 125000, 130000, 135000, 140000145000]
    };

    const cashFlowProjection = {
      inflows: [350000, 370000, 390000, 410000, 430000, 450000, 470000, 490000, 510000, 530000, 550000570000],
      outflows: [240000, 250000, 260000, 270000, 280000, 290000, 300000, 310000, 320000, 330000, 340000350000],
      net: [110000, 120000, 130000, 140000, 150000, 160000, 170000, 180000, 190000, 200000, 210000220000],
      cumulativeCash: [600000, 720000, 850000, 990000, 1140000, 1300000, 1470000, 1650000, 1840000, 2040000, 22500002470000]
    };

    // Format data for response
    const revenueData = displayMonths.map((monthi: any) => ({
      month,
    ...
    const profitData = displayMonths.map((monthi: any) => ({
      month,
    ...
    const cashFlowData = displayMonths.map((monthi: any) => ({
      month,
    ...
    return NextResponse.json({
      ...
      sensitivityAnalysis,
      summary: {
        projectedRevenue: revenueProjection.baseline.reduce((sumval: any) => sum + val0),
        projectedProfit: profitProjection.baseline.reduce((sumval: any) => sum + val0),
        projectedCashFlow: cashFlowProjection.net.reduce((sumval: any) => sum + val0),
