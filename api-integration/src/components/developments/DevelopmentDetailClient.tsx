...
// Removed direct Prisma import to avoid browser errors
...
export default function DevelopmentDetailClient({ initialDevelopmentId }: DevelopmentDetailClientProps) {
  ...
  // Real-time sync (temporarily disabled for debugging)
  // const {
  //   units: syncedUnits,
  //   lastUpdate,
  //   isLive,
  ...
  // Mock sync data for debugging
  const syncedUnits: any[] = [];
  const lastUpdate = null;
  const isLive = false;
  const initializeUnits = () => {};
  const refreshUnits = () => {};
  const hasActiveUpdate = () => false;
  const getActiveUpdate = () => null;
  ...
  useEffect(() => {
    const loadDevelopmentData = async () => {
      try {
        ...
        const devResponse = await fetch(`/api/developments/${developmentId}`);
        if (!devResponse.ok) {
          setError('Development not found');
          return;
        }
        const devResult = await devResponse.json();
        const devData = devResult.data || devResult; // Handle both nested and direct response
        setDevelopment(devData);
        
        // Load units for this development via API
        const unitsResponse = await fetch(`/api/developments/${developmentId}/units`);
        if (unitsResponse.ok) {
          const unitsData = await unitsResponse.json();
          setUnits(unitsData || []);
        } else {
          // Set empty array if units API fails
          setUnits([]);
        }
          ...
          <div className="mb-12">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex items-center space-x-4">
                ...
                >
                  {viewMode === 'grid' ? <List className="h-5 w-5 mr-2" /> : <Grid className="h-5 w-5 mr-2" />}
