...
describe('Authentication Security Tests', () => {
  ...
  describe('Authentication Flow Security', () => {
    it('should safely handle authentication errors without leaking details', async () => {
      ...
      mockAuth.signIn.mockRejectedValueOnce(
        new Error('NotAuthorizedException: Incorrect username or password [hash=1a2b3c4d, attempt=3, ip=192.168.1.1]')
      );
    ...
    it('should enforce secure password requirements', async () => {
      ...
      await act(async () => {
        try {
          await result.current.signUp('test@example.com', 'short', { // Too short
            name: 'Test User'
          });
        } catch (error) {
          // Expected error
  ...
  describe('Token Handling Security', () => {
    ...
    it('should handle token expiration gracefully', async () => {
      ...
      mockAuth.getCurrentUser
        ...
        .mockRejectedValueOnce(new Error('TokenExpiredException: Token has expired'));
  ...
  describe('CSRF Protection', () => {
    it('should include CSRF protection for state-changing operations', async () => {
      ...
      // Test if security-related methods are available (specific implementations may vary)
      // Skip this test if the method is not available
      if (typeof result.current.getSecurityHeaders === 'function') {
        const headers = await result.current.getSecurityHeaders();
