...
export function createMockAuth(options) {
    const { initialUser = null, signInBehavior = 'success', signUpBehavior = 'success', } = options || {};
    ...
    const authMock = {
        ...
        signUp: jest.fn(async ({ username, password, email }) => {
        ...
        resetPassword: jest.fn(async ({ username }) => {
            return {
        ...
        confirmResetPassword: jest.fn(async ({ username, confirmationCode, newPassword }) => {
            if (confirmationCode !== '123456') {
                throw new AuthError({
                    name: 'CodeMismatchException',
                    message: 'Invalid verification code provided',
...
export function createMockApi() {
    ...
    const apiMock = {
        ...
        __mockResponse: (method, path, data, options) => {
            const key = `${method.toUpperCase()}:${path}`;
            responseMap[key] = { data };
        },
        __mockGraphQLResponse: (queryName, data, options) => {
            const key = `GRAPHQL:${queryName}`;
            responseMap[key] = { data: { data } };
        },
        __mockError: (method, path, error) => {
            const key = `${method.toUpperCase()}:${path}`;
            responseMap[key] = { data: null, error };
        },
        __mockGraphQLError: (queryName, error) => {
            const key = `GRAPHQL:${queryName}`;
            responseMap[key] = {
                data: {
                    data: null,
...
export function createMockStorage() {
    ...
    const storageMock = {
        ...
        // Mock implementations
        getUrl: jest.fn(async ({ key }) => {
            if (!fileMap[key]) {
                return { url: '' };
            }
            return { url: fileMap[key].url || `https://mock-s3.amazonaws.com/${key}` };
        }),
        uploadData: jest.fn(async ({ key, data, options }) => {
            fileMap[key] = {
                data,
        ...
        downloadData: jest.fn(async ({ key }) => {
            if (!fileMap[key]) {
 ...
 */
export function simulateAuthenticatedUser(userData = {}) {
    const auth = require('aws-amplify/auth');
    const user = {
        userId: 'test-user-id',
        username: 'testuser',
 ...
 */
export function mockApiEndpoint(method, path, responseData) {
    const api = require('aws-amplify/api');
 ...
 */
export function mockGraphQLOperation(operationName, responseData) {
    const api = require('aws-amplify/api');
    // Configure the mock response
 ...
 */
export function mockApiError(method, path, error) {
    const api = require('aws-amplify/api');
 ...
 */
export function mockStorageFile(key, data, url) {
    const storage = require('aws-amplify/storage');
