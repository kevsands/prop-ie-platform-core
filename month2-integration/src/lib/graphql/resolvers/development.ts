...
export const developmentResolvers = {
  Query: {
    ...
    myDevelopments: async (_: any, 
      ...
      requireRole(context, [UserRole.DEVELOPER]);
      ...
      // Filter by published status
      if (filter?.isPublished !== undefined) {
        filteredDevelopments = filteredDevelopments.filter(dev => dev.isPublished === filter.isPublished);
      }
  ...
  Mutation: {
    ...
    addProjectMilestone: async (_: any, 
      ...
      requireRole(context, [UserRole.DEVELOPER, UserRole.ADMIN, UserRole.PROJECT_MANAGER]);
      ...
      const isAllowed = 
        ...
        (context.userRoles.includes(UserRole.PROJECT_MANAGER) && 
          development.professionalTeam.some(member => 
