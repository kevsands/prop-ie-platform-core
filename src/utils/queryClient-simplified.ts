...
const defaultOptions: DefaultOptions = {
  queries: {
    ...
    gcTime: 1000 * 60 * 10, // 10 minutes cache retention (renamed from cacheTime in v4)
    retry: 1, // Retry failed queries once
    retryDelay: (attempt: number) => Math.min(1000 * 2 ** attempt30000), // Exponential backoff
...
export function createQueryClient(options: {
  ...
  const {
    onError,
    defaultOptions: customDefaultOptions} = options;
...
export const queryUtils = {
  ...
  invalidatePropertyQueries: (queryClient: QueryClient, propertyId?: string) => {
    if (propertyId) {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.PROPERTY_DETAILpropertyId] });
    } else {
  ...
  invalidateDevelopmentQueries: (queryClient: QueryClient, developmentId?: string) => {
    if (developmentId) {
      queryClient.invalidateQueries({ queryKey: [QUERY_KEYS.DEVELOPMENT_DETAILdevelopmentId] });
    } else {
