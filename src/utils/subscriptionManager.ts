...
export class SubscriptionManager {
  ...
  private async connect(): Promise<void> {
    ...
    this.connectionPromise = new Promise<void>((resolvereject: any) => {
      try {
  ...
  private handleMessage = (event: MessageEvent) => {
    try {
      ...
      this.dispatchMessage(topicpayload);
    } catch (error) {
    ...
    subscriptionIds.forEach(id => {
      ...
      if (queryKeys && queryKeys.length> 0) {
        if (batchUpdates) {
          ...
          // Update immediately
          this.updateQueryCache(queryKeystransformedData);
        }
      }
  ...
  private batchQueryUpdate(
    ...
    if (!this.pendingBatches.has(batchKey)) {
      const timeout = setTimeout(() => {
        ...
        if (batch) {
          ...
          batch.updates.forEach((updateDataupdateKey: any) => {
            const [subIdqkIndex] = updateKey.split(':');
            const subscription = this.subscriptions.get(subId);
            if (subscription && subscription.options.queryKeys) {
              const queryKey = subscription.options.queryKeys[parseInt(qkIndex10)];
              this.updateSingleQueryCache(queryKeyupdateData);
            }
          });
    ...
    const batch = this.pendingBatches.get(batchKey)!;
    queryKeys.forEach((queryKeyindex: any) => {
      const updateKey = `${subscriptionId}:${index}`;
      batch.updates.set(updateKeydata);
    });
  }
  ...
  private updateQueryCache(queryKeys: unknown[][], data: any) {
    queryKeys.forEach(queryKey => {
      this.updateSingleQueryCache(queryKeydata);
    });
  }
  ...
  private updateSingleQueryCache(queryKey: unknown[], data: any) {
    ...
    if (Array.isArray(existing)) {
      ...
      if (data.id) {
        ...
        // Simple append
        queryClient.setQueryData(queryKey, (old: any[] = []) => [...olddata]);
      }
      ...
      queryClient.setQueryData(queryKeydata);
    }
  }
  ...
  private resubscribeAll() {
    ...
    this.subscriptions.forEach(subscription => {
      if (subscription.status === SubscriptionStatus.RECONNECTING || 
        ...
        topicSubscriptions.set(subscription.topicsubs);
      }
    });

    // Send subscription messages by topic
    topicSubscriptions.forEach((subscriptionIdstopic: any) => {
      try {
  ...
  public subscribe<T = any>(options: SubscriptionOptions<T>): Subscription<T> {
    const { topic } = options;
    const subscriptionId = `sub_${Date.now()}_${Math.random().toString(36).substring(29)}`;
    ...
    // Store subscription
    this.subscriptions.set(subscriptionIdsubscription);
...
export function createSubscriptionHook(wsUrl: string) {
  ...
  function useSubscription<T>(options: SubscriptionOptions<T>) {
    ...
    const [statussetStatus] = useState<SubscriptionStatus>(SubscriptionStatus.CONNECTING);
    const [datasetData] = useState<T | undefined>(undefined);
