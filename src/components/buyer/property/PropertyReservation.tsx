...
export default function PropertyReservation({ 
  ...
  const [reservationIdsetReservationId] = useState('');
  const [showDepositInfosetShowDepositInfo] = useState(false);
  const [showTimelinesetShowTimeline] = useState(false);
  const [reservationDetailssetReservationDetails] = useState<ReservationDetails>({
    fullName: '',
    email: '',
    phone: '',
  ...
  const getAvailableAppointmentDates = () => {
    ...
    while (dates.length <5) {
      currentDate = addDays(currentDate1);
                ...
                <Tabs defaultValue="dates">
                  ...
                  <TabsContent value="dates" className="space-y-4">
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-5 gap-2 mt-3">
                      {getAvailableAppointmentDates().map((dateindex: any) => (
                        <button
                          key={index}
                          type="button"
                    ...
                    {reservationDetails.appointmentDate && (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-2 mt-4">
                        {appointmentTimes.map((timeindex: any) => (
                          <button
                            key={index}
                            type="button"
