...
export default function ReadinessChecklist({ 
  ...
  const [categoryProgress, setCategoryProgress] = useState<Record<ChecklistCategory, number>>({
    financial: 0,
    documents: 0,
    research: 0,
    preparation: 0
  ...
  // Helper function to get dynamic color classes safely
  const getCategoryColorClass = (category: ChecklistCategory, type: 'bg' | 'border' | 'text') => {
    const colorName = categoryMeta[category].color;
    const intensity = type === 'bg' ? '50' : type === 'border' ? '200' : '600';
    return `${type}-${colorName}-${intensity}`;
  };
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mt-4">
            {(Object.keys(categoryMeta) as ChecklistCategory[]).map(category => {
              ...
              return (
                <div 
                  key={category}
                  className={`p-3 rounded-lg border ${borderColorClass} ${bgColorClass}`}
                >
                  <div className="flex items-center mb-2">
                    <div className={`w-6 h-6 rounded-full flex items-center justify-center mr-2 ${getCategoryColorClass(category, 'bg')}`}>
                      {meta.icon}
                    </div>
                  ...
                  <div className="flex items-center">
                    <Progress 
                      value={progress} 
                      className={`h-1.5 flex-1 mr-2 ${getCategoryColorClass(category, 'bg')}`}
                    />
          ...
          {(Object.keys(categoryMeta) as ChecklistCategory[]).map(category => {
            ...
            return (
              <div key={category} className="mb-4 border rounded-lg overflow-hidden">
                <div 
                  className={`flex items-center justify-between p-4 cursor-pointer hover:bg-gray-50 border-b ${borderColorClass} ${bgColorClass}`}
                  onClick={() => toggleCategory(category)}
                >
                  <div className="flex items-center">
                    <div className={`w-8 h-8 rounded-full ${bgColorClass} flex items-center justify-center mr-3`}>
                      {meta.icon}
                    </div>
                    <div>
