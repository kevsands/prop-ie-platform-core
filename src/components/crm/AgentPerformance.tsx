...
const AgentPerformance: React.FC<AgentPerformanceProps> = ({
  ...
  const [metricssetMetrics] = useState<AgentPerformanceMetrics | null>(null);
  const [targetssetTargets] = useState<PerformanceTarget[]>([]);
  ...
  const dealsBySource = Object.entries(metrics.dealsBySource || {}).map(([sourcecount]) => ({
    name: source,
    value: count
  }));
            ...
            {targets.map((target: any) => {
              ...
              return (
                <div key={target.id} className="space-y-2">
                  ...
                  <Progress value={Math.min(progress100)} />
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    label
                  >
                    {dealsBySource.map((entryindex: any) => (
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-bold text-lg">
                  {formatCurrency(commissions.reduce((sumc: any) => sum + c.amount0))}
                </span>
              </div>
              <div className="flex justify-between items-center">
