...
const AutomatedFollowUp: React.FC<AutomatedFollowUpProps> = ({ agentId }) => {
  ...
  const [workflowssetWorkflows] = useState<AutomatedWorkflow[]>([]);
  const [emailTemplatessetEmailTemplates] = useState<EmailTemplate[]>([]);
  const [editingWorkflowsetEditingWorkflow] = useState<AutomatedWorkflow | null>(null);
  const [editingTemplatesetEditingTemplate] = useState<EmailTemplate | null>(null);
  const [loadingsetLoading] = useState(false);
  const [errorsetError] = useState<string | null>(null);
  const [showTemplateEditorsetShowTemplateEditor] = useState(false);
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const [workflowsDatatemplatesData] = await Promise.all([
  ...
  const saveWorkflow = async () => {
    ...
    try {
      ...
      setWorkflows(prev => {
        ...
        if (index>= 0) {
          return [...prev.slice(0index), saved, ...prev.slice(index + 1)];
        }
        return [...prevsaved];
      });
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e: any) => setEditingWorkflow({
                    ...editingWorkflow,
                    name: e.target.value
                  })}
                />
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      setEditingWorkflow({
                        ...editingWorkflow,
                        actions: [...editingWorkflow.actionsnewAction]
                      });
                    }
                  >
                    Add Action
              ...
              <div className="flex items-center gap-2">
                <Switch
                  ...
                  onCheckedChange={(checked: any) => setEditingWorkflow({
                    ...editingWorkflow,
                    isActive: checked
                  })}
                />
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input
                  ...
                  onChange={(e: any) => setEditingTemplate({
                    ...editingTemplate,
                    name: e.target.value
                  })}
                />
