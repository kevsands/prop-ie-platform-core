...
const CommissionTracking: React.FC<CommissionTrackingProps> = ({
  ...
  const loadCommissionData = async () => {
    ...
    try {
      ...
      // Calculate totals
      const total = commissionsData.reduce((sumc: any) => sum + c.amount0);
      const pending = commissionsData
        .filter(c => c.status === 'pending')
        .reduce((sumc: any) => sum + c.amount0);
  ...
  const calculateMonthlyEarnings = (commissions: Commission[]) => {
    ...
    return Array.from(monthlyMap.entries())
      ...
      .sort((ab: any) => new Date(a.month).getTime() - new Date(b.month).getTime());
  };
  ...
  const addCommissionTier = () => {
    ...
    const updatedStructure = {
      ...commissionStructure,
      tiers: [...commissionStructure.tiersnewTier].sort((ab: any) => a.minAmount - b.minAmount)
    };
  ...
  const removeTier = (index: number) => {
    ...
    const updatedStructure = {
      ...commissionStructure,
      tiers: commissionStructure.tiers.filter((_i: any) => i !== index)
    };
            ...
            <div className="space-y-4">
              ...
              {editingStructure ? (
                <div className="space-y-4 border-t pt-4">
                  ...
                  {commissionStructure.tiers.map((tierindex: any) => (
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {commissions.slice(010).map((commission: any) => (
