...
export const CommentThread: React.FC<CommentThreadProps> = ({
  ...
  const [commentssetComments] = useState<Comment[]>([]);
  const [newCommentsetNewComment] = useState('');
  const [replyingTosetReplyingTo] = useState<string | null>(null);
  const [editingCommentsetEditingComment] = useState<string | null>(null);
  const [editContentsetEditContent] = useState('');
  ...
  useEffect(() => {
    ...
    const interval = setInterval(fetchComments5000);
  ...
  const handleSubmitComment = async () => {
    ...
    try {
      ...
      if (replyingTo) {
        ...
        setComments(prevComments => [...prevCommentscomment]);
      }
    ...
    >
      <div className="flex space-x-3 p-4">
        ...
        <div className="flex-1">
          ...
          {comment.replies && comment.replies.map(reply => renderComment(replytrue))}
