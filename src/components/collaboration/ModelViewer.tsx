...
export const ModelViewer: React.FC<ModelViewerProps> = ({ projectId, modelUrl }) => {
  const [zoomsetZoom] = useState(100);
  const [rotationsetRotation] = useState({ x: 0, y: 0, z: 0 });
  const [showGridsetShowGrid] = useState(true);
  const [showWireframesetShowWireframe] = useState(false);
  const [selectedLayersetSelectedLayer] = useState('all');
  const [loadingsetLoading] = useState(false);
  const [modelStatssetModelStats] = useState({
    vertices: 0,
    faces: 0,
    materials: 0,
    textures: 0
  ...
  const handleZoomIn = () => {
    setZoom(Math.min(zoom + 10200));
  };

  const handleZoomOut = () => {
    setZoom(Math.max(zoom - 1050));
  };
          ...
          <Card>
            ...
            <CardContent>
              <div className="space-y-2">
                {['All Layers', 'Structure', 'MEP', 'Interior', 'Exterior'].map((layer: any) => (
                  <label
                    key={layer}
                  ...
                  >
                    ...
                    <input
                      ...
                      onChange={(e: any) => setSelectedLayer(e.target.value)}
                      className="h-4 w-4"
                    />
                  </label>
                ))}
