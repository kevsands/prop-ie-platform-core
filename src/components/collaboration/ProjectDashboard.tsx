...
export const ProjectDashboard: React.FC<ProjectDashboardProps> = ({ projectId }) => {
  const [projectsetProject] = useState<DialogCollaborationProject | null>(null);
  const [statssetStats] = useState<ProjectStats>({
    totalTasks: 0,
    completedTasks: 0,
    pendingDrawings: 0,
    recentActivity: 0
  });
  const [recentTaskssetRecentTasks] = useState<DialogTask[]>([]);
