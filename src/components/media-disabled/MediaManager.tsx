...
const MediaManager: React.FC<MediaManagerProps> = ({
  ...
  const [activeCategorysetActiveCategory] = useState<string>(defaultCategory);
  const [uploadOpensetUploadOpen] = useState<boolean>(false);
  ...
  const handleSaveFiles = useCallback(() => {
    if (onAddMedia && mediaFiles.length> 0) {
      onAddMedia(mediaFilesactiveCategory);
      setMediaFiles([]);
      setUploadOpen(false);
    }
  }, [mediaFilesactiveCategoryonAddMedia]);
