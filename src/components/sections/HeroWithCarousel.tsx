...
export const HeroWithCarousel = () => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setCurrentImageIndex((prev: any) => (prev + 1) % heroImages.length);
  ...
  const prevImage = () => {
    setCurrentImageIndex((prev: any) => (prev - 1 + heroImages.length) % heroImages.length);
  };
            ...
            <div className="flex-1 relative">
              <div className={`relative flex items-center transition-all duration-200 ${
                ...
                <input
                  ...
                  onKeyDown={(e: any) => e.key === 'Enter' && router.push('/properties/search')}
                />
