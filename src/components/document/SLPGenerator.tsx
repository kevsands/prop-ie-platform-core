...
const SLPGenerator: React.FC<SLPGeneratorProps> = ({
  ...
  const [selectedTemplatesetSelectedTemplate] = useState<SLPTemplate | null>(null);
  const [isGeneratingsetIsGenerating] = useState(false);
  const [generationProgresssetGenerationProgress] = useState(0);
  const [selectedDocumentssetSelectedDocuments] = useState<Set<string>>(new Set());
  const [batchModesetBatchMode] = useState(false);
  const [complianceCheckResultssetComplianceCheckResults] = useState<any>(null);
  const [previewModesetPreviewMode] = useState(false);
                ...
                <CardContent>
                  <div className="flex items-center space-x-2">
                    <Checkbox 
                      id="batch-mode"
                      checked={batchMode}
                      onCheckedChange={(checked: any) => setBatchMode(checked as boolean)}
                    />
                ...
                <>
                  <Card>
                    ...
                    <CardContent className="space-y-2">
                      {selectedTemplate.requiredDocuments.map((doc: any) => {
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-2">
                      {selectedTemplate.optionalDocuments.map((doc: any) => {
                        const status = getDocumentStatus(doc);
                        return (
                          <div key={doc} className="flex items-center justify-between p-2 rounded-lg hover:bg-gray-50">
                            <div className="flex items-center space-x-3">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <ol className="list-decimal list-inside space-y-1 text-sm">
                          {Array.from(selectedDocuments).map((docindex: any) => (
