...
const DocumentIntegrations: React.FC<DocumentIntegrationsProps> = ({
  ...
  const [operationProgresssetOperationProgress] = useState(0);
  const [activeOperationsetActiveOperation] = useState<string | null>(null);

  // Email state
  const [emailDialogOpensetEmailDialogOpen] = useState(false);
  const [emailRecipientssetEmailRecipients] = useState<EmailRecipient[]>([]);
  const [emailSubjectsetEmailSubject] = useState('');
  const [emailMessagesetEmailMessage] = useState('');

  // Bulk operation state
  const [bulkOperationsetBulkOperation] = useState<string>('');
  const [bulkCategorysetBulkCategory] = useState('');
  const [bulkTagssetBulkTags] = useState<string[]>([]);

  // Archive state
  const [archiveDialogOpensetArchiveDialogOpen] = useState(false);
  const [archiveSettingssetArchiveSettings] = useState<ArchiveSettings>({
    retentionPeriod: 365,
    compressFiles: true,
    encryptArchive: true,
    notifyOwners: true
  ...
  const removeEmailRecipient = (index: number) => {
    setEmailRecipients(emailRecipients.filter((_i: any) => i !== index));
  };
          ...
          {bulkOperation === 'add-tags' && (
            <div>
              ...
              <Input
                ...
                onChange={(e: any) => setBulkTags(e.target.value.split(',').map(t => t.trim()))}
              />
            </div>
          )}
          ...
          <div className="space-y-4">
            <div>
              ...
              <div className="space-y-2">
                {emailRecipients.map((recipientindex: any) => (
                  <div key={index} className="flex space-x-2">
                    <Input
                      type="email"
                      placeholder="email@example.com"
                      value={recipient.email}
                      onChange={(e: any) => updateEmailRecipient(index, e.target.value)}
                    />
                    <Button
                      variant="ghost"
                      size="icon"
          ...
          <div className="space-y-4">
            <div>
              ...
              <Select 
                ...
                onValueChange={(value: any) => setArchiveSettings({
                  ...archiveSettings,
            ...
            <div className="space-y-2">
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked: any) => setArchiveSettings({
                    ...archiveSettings,
