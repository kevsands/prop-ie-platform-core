...
export function BOQQuickView({ projectId, selectedItem }: BOQQuickViewProps) {
  ...
  const [quickCalculationsetQuickCalculation] = useState({
    quantity: '',
    rate: '',
    total: 0
  });
  ...
  const totalBOQValue = mockBOQItems.reduce((sumitem) => sum + item.amount0);
  const averageVariance = mockBOQItems.reduce((sumitem) => sum + Math.abs(item.variance), 0) / mockBOQItems.length;
