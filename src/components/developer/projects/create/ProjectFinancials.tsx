...
export default function ProjectFinancials({ data, onNext, onBack }: ProjectFinancialsProps) {
  const [contingencysetContingency] = useState(data?.contingency || 10);
  const [profitMarginsetProfitMargin] = useState(data?.profitMargin || 20);
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div>
              ...
              <Select onValueChange={(value: any) => setValue('financingType', value)}>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    ...
                    cy="50%"
                    labelLine={false}
                    label={(entry: any) => `${entry.name}: €${(entry.value / 1000000).toFixed(1)}M`}
                    outerRadius={80}
                    fill="#8884d8"
                    dataKey="value"
                  >
                    {costBreakdown.map((entryindex: any) => (
