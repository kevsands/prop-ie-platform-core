...
export function BuyerJourney({ developmentId, unit, onComplete }: BuyerJourneyProps) {
  ...
  // State
  const [currentStepsetCurrentStep] = useState<JourneyStep>('authentication');
  const [isVerifiedsetIsVerified] = useState(false);
  const [isReservingsetIsReserving] = useState(false);
  const [errorsetError] = useState<string | null>(null);
  const [transactionIdsetTransactionId] = useState<string | null>(null);
  const [mockTransactionIdsetMockTransactionId] = useState<string>('');
  ...
  useEffect(() => {
    ...
    if (isAuthenticated && currentStep === 'authentication') {
      ...
      const newMockId = `txn-${Math.random().toString(36).substring(210)}`;
  ...
  }, [mockTransactionIdstartKYCMutation]);
