...
export function useCustomization(propertyId: string) {
  ...
  const { data: savedCustomization, isLoading, error } = useQuery({
    queryKey: [CUSTOMIZATION_KEYpropertyId],
    queryFn: async () => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: (data: any) => {
      // Update cache
      queryClient.setQueryData([CUSTOMIZATION_KEYpropertyId], data);
  ...
  const calculatePriceMutation = useMutation({
    ...
    onSuccess: (datavariables: any) => {
      ...
      queryClient.setQueryData(cacheKeydata);
    });
