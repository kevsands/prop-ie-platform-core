...
export function useSecurityPerformance(
  ...
  // Calculated metrics
  const [memoryEfficiency, setMemoryEfficiency] = useState<number>(100);
  const [cacheEffectiveness, setCacheEffectiveness] = useState<number>(0);
  const [workerUtilization, setWorkerUtilization] = useState<number>(0);
  
  // Tracking metrics over time
  const metricsHistory = useRef<any[]>([]);
  ...
  const updatePerformanceMetrics = useCallback(async () => {
    try {
      ...
      const metrics = await SecurityAnalytics.getPerformanceMetrics();
      ...
      console.error('Error fetching security performance metrics:', err);
  ...
  const calculateDerivedMetrics = useCallback((metrics: any) => {
    ...
    if (metricsHistory.current.length >= 2) {
      ...
      const effectiveness = Math.min(100, Math.max(0, 
        (latencyImprovement * 100) - (Math.max(0, cacheGrowthRate) * 50) + 50
      ));
  ...
  }, [immediateInitialFetch, refreshInterval, updatePerformanceMetrics]);
  
  // Get history metrics (last n items)
  const getHistoryMetrics = useCallback((count = 10) => {
    return metricsHistory.current.slice(0, count);
  }, []);
  ...
  const calculateTrend = useCallback((metricPath: string[], count = 5): 'up' | 'down' | 'stable' => {
    ...
    const values = metricsHistory.current
      ...
      .map(metrics => {
        ...
        return metricPath.reduce((obj, key) => obj && obj[key], metrics as any);
      })
