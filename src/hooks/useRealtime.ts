...
export function useRealtime(config: RealtimeConfig = {}) {
  const { user } = useAuth();
  const [statesetState] = useState<RealtimeState>({
    connected: false,
    connecting: false,
    error: null,
    rooms: new Map(),
...
export function useTransactionRealtime(transactionId: string) {
  ...
  }, [realtime.connectedtransactionId]);
...
export function useDevelopmentRealtime(developmentId: string) {
  ...
  }, [realtime.connecteddevelopmentId]);
