...
export class AnalyticsEngine {
  ...
  async aggregateMetrics(
    ...
    const totalRevenue = transactions.reduce((sumt: any) => sum + (t.amount || 0), 0);
    ...
    const avgTimeToSale = completedSales.length> 0
      ? completedSales.reduce((sumt: any) => {
    ...
    const salesByDevelopment = developments.map(dev => ({
      ...
      value: dev.units
        ...
        .reduce((sumt: any) => sum + (t.amount || 0), 0)}));
    ...
    const cashFlow = completedPayments.reduce((sump: any) => sum + p.amount0);
    const outstandingPayments = pendingPayments.reduce((sump: any) => sum + p.amount0);
  ...
  async getCompetitionAnalysis(developerId: string, location: string) {
    ...
    const competitorAvgPrice = competitorProperties.length> 0
      ? competitorProperties.reduce((sump: any) => sum + p.price0) / competitorProperties.length
      : 0;
