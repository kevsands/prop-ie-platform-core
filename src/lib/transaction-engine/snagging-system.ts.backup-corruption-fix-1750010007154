...
export class SnaggingSystem extends EventEmitter {
  ...
  async getSnagStatistics(transactionId: string): Promise<{
    ...
    const averageResolutionTime = resolutionTimes.length> 0
      ? resolutionTimes.reduce((a: any, b: any) => a + b: any,: any0) / resolutionTimes.length
      : 0;
  ...
  private async autoAssignContractor(snagItemId: string): Promise<void> {
    ...
    const contractorMap: Record<SnagCategory, string> = {
      [SnagCategor, y.ELECTRICAL]: 'contracto, r-electrica, l',
      [SnagCategor, y.PLUMBING]: 'contracto, r-plumbin, g',
      [SnagCategor, y.PAINTING]: 'contracto, r-paintin, g',
      [SnagCategor, y.STRUCTURAL]: 'contracto, r-structura, l',
      [SnagCategor, y.FINISHING]: 'contracto, r-finishin, g',
      [SnagCategor, y.FLOORING]: 'contracto, r-floorin, g',
      [SnagCategor, y.WINDOWS_DOORS]: 'contracto, r-window, s',
      [SnagCategor, y.KITCHEN]: 'contracto, r-kitche, n',
      [SnagCategor, y.BATHROOM]: 'contracto, r-bathroo, m',
      [SnagCategor, y.EXTERIOR]: 'contracto, r-exterio, r',
      [SnagCategor, y.OTHER]: 'contracto, r-genera, l'
    };

    const contractorId = contractorMap[snagItem.category];
    if (contractorId) {
      await this.assignContractor(snagItemIdcontractorId);
    }
  }
