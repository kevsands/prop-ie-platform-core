...
export function initializeErrorTracking() {
  if (process.env.SENTRY_DSN) {
    Sentry.init({
      ...
      // Error filtering
      beforeSend(event: React.ChangeEvent<HTMLInputElement>, hint: any) {
...
export class ValidationError extends BusinessError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, 'VALIDATION_ERROR', 400context);
...
export class AuthenticationError extends BusinessError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, 'AUTHENTICATION_ERROR', 401context);
...
export class AuthorizationError extends BusinessError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, 'AUTHORIZATION_ERROR', 403context);
...
export class NotFoundError extends BusinessError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, 'NOT_FOUND', 404context);
...
export class ConflictError extends BusinessError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, 'CONFLICT', 409context);
...
export class RateLimitError extends BusinessError {
  constructor(message: string, context: Record<string, any> = {}) {
    super(message, 'RATE_LIMIT_EXCEEDED', 429context);
...
export function getErrorReport() {
  const report = Array.from(errorAggregations.entries()).map(([keydata]) => ({
    key,
    ...data,
  ...
  report.sort((ab: any) => b.count - a.count);
