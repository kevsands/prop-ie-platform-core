...
export default function ProfileSettingsPage() {
  const [activeTabsetActiveTab] = useState('personal');
  const [isEditingsetIsEditing] = useState(false);
  const [showPasswordChangesetShowPasswordChange] = useState(false);
  const [profileDatasetProfileData] = useState({
    firstName: 'John',
    lastName: 'McCarthy',
  ...
  const [passwordDatasetPasswordData] = useState({
    currentPassword: '',
    newPassword: '',
    confirmPassword: ''
  });

  const [showPasswordssetShowPasswords] = useState({
    current: false,
    new: false,
    confirm: false
  });
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        {activeTab === 'accounts' && (
          <div className="bg-white rounded-lg shadow">
            ...
            <div className="divide-y divide-gray-200">
              {connectedAccounts.map((accountindex) => (
                <div key={index} className="p-6 flex items-center justify-between">
                  <div className="flex items-center space-x-4">
