...
export default function DeveloperTimelinePage() {
  const [activeViewsetActiveView] = useState('gantt');
  const [selectedProjectssetSelectedProjects] = useState(['all']);
  const [timeScalesetTimeScale] = useState('months');
  const [currentDatesetCurrentDate] = useState(new Date());
  ...
  const portfolioStats = {
    ...
    activePhases: projects.reduce((accproject) => acc + project.phases.filter(p => p.status === 'in_progress').length0),
    completedMilestones: projects.reduce((accproject) => acc + project.milestones.filter(m => m.status === 'completed').length0),
    upcomingDeadlines: 8,
    criticalPath: 'Fitzgerald Gardens - Structure Complete',
    portfolioProgress: Math.round(projects.reduce((accproject) => acc + project.progress0) / projects.length)
  };
