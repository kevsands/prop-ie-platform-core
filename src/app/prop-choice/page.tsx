...
export default function PropChoicePage() {
  const [selectedOptionssetSelectedOptions] = useState<Record<string, string>>({});
  const [activeCategorysetActiveCategory] = useState('flooring');
  const [showSummarysetShowSummary] = useState(false);

  // Calculate total cost
  const calculateTotal = () => {
    return Object.values(selectedOptions).reduce((totaloptionId: any) => {
  ...
  const getSelectedOptions = () => {
    return Object.entries(selectedOptions).map(([categoryIdoptionId]) => {
                        ...
                        <CardContent>
                          ...
                          {option.specifications && (
                            <ul className="text-sm text-muted-foreground space-y-1 mb-4">
                              {option.specifications.map((specindex: any) => (
