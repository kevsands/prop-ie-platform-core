...
export default function TestDeployment() {
  const [databaseStatussetDatabaseStatus] = useState<'loading' | 'connected' | 'error'>('loading');
  const [authStatussetAuthStatus] = useState<'loading' | 'configured' | 'error'>('loading');
  const [apiStatussetApiStatus] = useState<'loading' | 'online' | 'error'>('loading');
  const [envVarssetEnvVars] = useState<string[]>([]);
  const [statussetStatus] = useState<any>({});
