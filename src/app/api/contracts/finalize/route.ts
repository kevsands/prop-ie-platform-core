...
async function verifyDocuSignCompletion(envelopeId: string) {
  ...
  try {
    ...
    // const envelope = await envelopesApi.getEnvelope(accountIdenvelopeId);
    // const recipients = await envelopesApi.listRecipients(accountIdenvelopeId);
...
async function updateDepositStatus(reservationId: string, status: string) {
  ...
  // await escrowService.updateDepositStatus(reservationIdstatus);
