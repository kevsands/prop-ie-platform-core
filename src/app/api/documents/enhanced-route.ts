...
// Helper async function toprisma.user.findUnique({
    where: { id: userId },
    include: { roles: true });
...
// Helper async function toprisma.auditLog.create({
      data: {
        userId,
        action,
        resourceType,
...
async function scanForViruses(buffer: Buffer): Promise<{ clean: boolean; threats?: string[] }> {
  ...
  await new Promise(resolve => setTimeout(resolve500)); // Simulate scanning time
...
async function performOCR(buffer: Buffer, mimeType: string): Promise<{ text: string; confidence: number }> {
  ...
  await new Promise(resolve => setTimeout(resolve1000)); // Simulate OCR time
...
// Helper async function toemailTransporter.sendMail({
