...
function generatePDF(analytics: any, developerName: string) {
  ...
  // Title
  doc.setFontSize(20);
  doc.text('Developer Analytics Report', 1422);

  // Subtitle
  doc.setFontSize(12);
  doc.text(`Generated for: ${developerName}`, 1432);
  doc.text(`Date: ${format(new Date(), 'MMMM d, yyyy')}`, 1440);

  // KPI Summary
  doc.setFontSize(16);
  doc.text('Key Performance Indicators', 1455);
  ...
  doc.setFontSize(16);
  doc.text('Revenue by Development', 14yPos);
  ...
  doc.setFontSize(16);
  doc.text('Financial Summary', 14finYPos);
