...
export async function GET(req: NextRequest) {
  try {
    ...
    // Header
    doc.setFontSize(24);
    doc.text('Property Customization Report', 2020);

    doc.setFontSize(12);
    doc.text(`Date: ${new Date().toLocaleDateString()}`, 2030);
    doc.text(`Customer: ${session.user.name || session.user.email}`, 2037);

    // Property Details
    doc.setFontSize(16);
    doc.text('Property Details', 2050);

    doc.setFontSize(12);
    doc.text(`Property: ${customization.propertyName || 'N/A'}`, 2060);
    doc.text(`Base Price: €${customization.basePrice?.toLocaleString() || '0'}`, 2067);

    // Customization Summary
    doc.setFontSize(16);
    doc.text('Selected Customizations', 2085);
    ...
    if (customization.options) {
      Object.entries(customization.options).forEach(([categoryoptions]: [stringany]) => {
    ...
    doc.setFontSize(10);
    doc.text('This is a preliminary quote and subject to final approval.', 20280);
    doc.text('Valid for 30 days from the date of generation.', 20285);
