    ...
    switch (paymentData.status) {
      case 'INITIATED':
        ...
        const newPayment = await transactionService.processPayment(id, {
          ...
          reference: paymentData.reference || `PAY-${Date.now()}-${Math.random().toString(36).substring(27).toUpperCase()}`,
...
// Helper async function totransactionService.updateTransaction(
    transactionId,
    { status },
    performedBy
  );
