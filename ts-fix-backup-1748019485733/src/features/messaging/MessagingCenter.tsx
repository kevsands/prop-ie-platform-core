    ...
    <Sheet open={isOpen} onOpenChange={setIsOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="relative">
          ...
          {conversations.reduce((sumconv) => sum + conv.unreadCount0) > 0 && (
            <Badge 
              variant="destructive" 
              className="absolute -top-1 -right-1 h-5 w-5 p-0 text-xs"
            >
              {conversations.reduce((sumconv) => sum + conv.unreadCount0)}
            </Badge>
          )}
        </Button>
      </SheetTrigger>
            ...
            <Tabs value={selectedTab} onValueChange={setSelectedTab}>
              <TabsList className="w-full rounded-none border-b">
                ...
                <TabsTrigger value="unread" className="flex-1">
                  Unread ({conversations.reduce((sumconv) => sum + conv.unreadCount0)})
                </TabsTrigger>
                    ...
                    {attachments.map((fileindex) => (
                      <div key={index} className="relative group">
                        ...
                        <Button
                          ...
                          onClick={() => setAttachments(attachments.filter((_i) => i !== index))}
                        >
