...
export default function TransactionMilestones({ 
  ...
  const milestoneStats = useMemo(() => {
    const stats = {
      ...
      averageProgress: milestones.reduce((summ) => sum + m.progress0) / milestones.length
    };
          ...
          <CardContent className="p-6">
            <div className="relative">
              ...
              <div className="space-y-8">
                {filteredMilestones
                  .sort((ab) => a.dueDate.getTime() - b.dueDate.getTime())
                  .map((milestoneindex) => {
                    const daysUntilDue = differenceInDays(milestone.dueDate, new Date());
                    const isOverdue = milestone.status !== 'COMPLETED' && daysUntilDue <0;
                    ...
                    <tr key={milestone.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <div className="flex -space-x-2">
                          {milestone.responsible.slice(03).map((personidx) => (
