...
export class IndexedDBStorageAdapter implements StorageAdapter {
  ...
  private getDB(): Promise<IDBDatabase> {
    ...
    this.dbPromise = new Promise((resolvereject) => {
      try {
  ...
  async clear(): Promise<void> {
    try {
      ...
      const allKeys = await new Promise<string[]>((resolvereject) => {
        const transaction = db.transaction(this.storeName, 'readonly');
        const store = transaction.objectStore(this.storeName);
  ...
  async keys(): Promise<string[]> {
    try {
      ...
      return new Promise<string[]>((resolvereject) => {
        const transaction = db.transaction(this.storeName, 'readonly');
        const store = transaction.objectStore(this.storeName);
        const request = store.getAllKeys();
  ...
  private evictEntries(count: number): void {
    ...
    switch (this.options.evictionPolicy) {
      case 'lru': // Least Recently Used
        entries.sort((ab) => a[1].lastAccessed - b[1].lastAccessed);
        break;
      case 'lfu': // Least Frequently Used
        entries.sort((ab) => a[1].accessCount - b[1].accessCount);
        break;
      case 'fifo': // First In, First Out
        entries.sort((ab) => a[1].createdAt - b[1].createdAt);
        break;
    }
