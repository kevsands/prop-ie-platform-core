...
export async function GET(request: NextRequest) {
  try {
    ...
    const priceRangePerformance = priceRanges.map(range => {
      ...
      const totalViews = unitsInRange.reduce((sumunit) => sum + unit._count.propertyViews0);
    ...
    return NextResponse.json({
      ...
      competitionAnalysis: {
        avgCompetitorPrice: competitionData.length> 0
          ? competitionData.reduce((sumunit) => sum + unit.price0) / competitionData.length
          : 0,
        competitors: competitionData.slice(05)},
      recommendations: {
        priceOptimization: priceRangePerformance
          .filter(range => range.avgViews> 0)
          .sort((ab) => b.avgViews - a.avgViews)[0]?.range || 'No data',
