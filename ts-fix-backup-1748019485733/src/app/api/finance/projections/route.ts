...
export async function GET(request: NextRequest) {
  try {
    ...
    const revenueData = displayMonths.map((monthi) => ({
      month,
    ...
    const profitData = displayMonths.map((monthi) => ({
      month,
    ...
    const cashFlowData = displayMonths.map((monthi) => ({
      month,
    ...
    return NextResponse.json({
      ...
      sensitivityAnalysis,
      summary: {
        projectedRevenue: revenueProjection.baseline.reduce((sumval) => sum + val0),
        projectedProfit: profitProjection.baseline.reduce((sumval) => sum + val0),
        projectedCashFlow: cashFlowProjection.net.reduce((sumval) => sum + val0),
