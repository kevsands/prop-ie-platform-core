...
export async function GET(request: NextRequest) {
  try {
    ...
    const summary = {
      totalBudget: filteredData.reduce((sumitem) => sum + item.budgetValue0),
      totalActual: filteredData.reduce((sumitem) => sum + item.actualValue0),
      variance: filteredData.reduce((sumitem) => sum + (item.actualValue - item.budgetValue), 0),
