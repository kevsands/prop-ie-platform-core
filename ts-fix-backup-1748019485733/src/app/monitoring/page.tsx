...
export default function MonitoringDashboard() {
  const [healthDatasetHealthData] = useState<HealthData | null>(null);
  const [metricsDatasetMetricsData] = useState<MetricsData | null>(null);
  const [historicalDatasetHistoricalData] = useState<any[]>([]);
  ...
  const fetchData = async () => {
    try {
      const [healthResponsemetricsResponse] = await Promise.all([
        fetch('/api/health'),
        fetch('/api/metrics')
      ]);
      ...
      <Card className="p-6">
        ...
        <div className="overflow-x-auto">
          <table className="min-w-full">
            ...
            <tbody>
              {metricsData?.endpoints.map((endpointindex) => (
