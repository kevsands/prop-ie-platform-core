...
export default function DeveloperDashboard() {
  ...
  useEffect(() => {
    if (developments?.data) {
      ...
      const totalUnits = developments.data.reduce((accdev) => acc + dev.totalUnits0);
      const soldUnits = developments.data.reduce((accdev) => acc + dev.soldUnits0);
      const availableUnits = developments.data.reduce((accdev) => acc + dev.availableUnits0);
      const reservedUnits = developments.data.reduce((accdev) => acc + dev.reservedUnits0);
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          {isLoadingDevelopments ? (
            ...
            developments?.data.slice(03).map((development) => (
