...
export function SnaggingInspection({ transactionId, onComplete }: SnaggingInspectionProps) {
  const [snagItemssetSnagItems] = useState<SnagItem[]>([]);
  const [schedulesetSchedule] = useState<InspectionSchedule | null>(null);
  const [loadingsetLoading] = useState(true);
  const [errorsetError] = useState<string | null>(null);
  const [showAddFormsetShowAddForm] = useState(false);
  const [uploadingImagesetUploadingImage] = useState(false);
  ...
  const fetchInspectionData = async () => {
    try {
      const [scheduleResitemsRes] = await Promise.all([
  ...
  const handleAddSnagItem = async () => {
    ...
    try {
      ...
      setSnagItems(prev => [...prevnewSnagItem]);
