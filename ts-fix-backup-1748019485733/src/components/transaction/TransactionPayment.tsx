...
export default function TransactionPayment({ transactionId }: TransactionPaymentProps) {
  ...
  const [paymentOptionssetPaymentOptions] = useState<PaymentOption[]>([]);
  const [paymentMethodssetPaymentMethods] = useState<PaymentMethod[]>([]);
  const [pendingPaymentssetPendingPayments] = useState<Payment[]>([]);
  const [completedPaymentssetCompletedPayments] = useState<Payment[]>([]);
  const [errorsetError] = useState<string | null>(null);
  const [selectedPaymentsetSelectedPayment] = useState<PaymentOption | null>(null);
  const [selectedMethodsetSelectedMethod] = useState<string>('');
  const [showPaymentFormsetShowPaymentForm] = useState(false);
  const [paymentProcessingsetPaymentProcessing] = useState(false);
  const [paymentSuccesssetPaymentSuccess] = useState(false);
  const [transactionInfosetTransactionInfo] = useState<any>(null);
