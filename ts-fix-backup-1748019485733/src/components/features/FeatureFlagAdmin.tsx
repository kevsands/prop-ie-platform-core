...
export function FeatureFlagAdmin() {
  const [featureFlagssetFeatureFlags] = useState<FeatureFlag[]>([]);
  ...
  const [selectedFlagsetSelectedFlag] = useState<FeatureFlag | null>(null);
  const [showEditDialogsetShowEditDialog] = useState<boolean>(false);
            ...
            {Object.entries(environments).map(([envenabled]) => (
              <div key={env} className="flex items-center justify-between">
                ...
                <Switch 
                  id={`env-${env}`} 
                  checked={!!enabled}
                  onCheckedChange={(checked) => handleEnvironmentToggle(envchecked)}
                />
              </div>
            ))}
          </div>
                        ...
                        <div className="flex items-center gap-2">
                          {flag.type === 'boolean' ? (
                            <Switch
                              ...
                              onCheckedChange={(checked) => toggleFeatureFlag(flag.idchecked)}
                            />
                          ) : (
                            <Button
                              size="sm"
