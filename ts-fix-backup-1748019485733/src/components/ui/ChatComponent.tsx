...
const ChatComponent: React.FC<ChatComponentProps> = ({
  ...
  const groupedMessages: { [date: string]: ChatMessage[] } = messages.reduce((groupsmessage) => {
  ...
  const removeAttachment = (index: number) => {
    setAttachments(attachments.filter((_i) => i !== index));
  };
  ...
  return (
    <div className={`flex flex-col border rounded-lg overflow-hidden h-[600px] bg-white ${className}`}>
      ...
      <div className="p-3 border-b bg-gray-50 flex justify-between items-center">
        ...
        {showParticipants && (
          <div className="flex">
            {participants.slice(03).map((participantindex) => (
              <div 
                key={participant.id} 
