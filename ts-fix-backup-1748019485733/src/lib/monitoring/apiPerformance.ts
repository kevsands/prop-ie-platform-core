...
function estimateTypicalDuration(endpoint: string, method: string): number {
  ...
  const durations = relevantMetrics.map(m => m.duration).sort((ab) => a - b);
...
export function getPerformanceSummary() {
  ...
  const totalDuration = metricsCache.reduce((summetric) => sum + metric.duration0);
  ...
  Object.entries(endpointMetrics).forEach(([endpointmetrics]) => {
    ...
    const totalDuration = metrics.reduce((summ) => sum + m.duration0);
