...
export class NotificationService extends EventEmitter {
  ...
  private subscribeToTransactionEvents(): void {
    transactionEngine.on(TransactionEvent.STATE_CHANGED, async (data) => {
      await this.handleTransactionStateChange(data);
    });

    transactionEngine.on(TransactionEvent.PAYMENT_RECEIVED, async (data) => {
      await this.handlePaymentReceived(data);
    });

    transactionEngine.on(TransactionEvent.DOCUMENT_UPLOADED, async (data) => {
  ...
  private getNestedValue(obj: any, key: string): any {
    return key.split('.').reduce((ok) => o?.[k], obj);
  }
