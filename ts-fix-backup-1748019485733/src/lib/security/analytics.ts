...
class AnalyticsCache {
  ...
  cleanup(): void {
    ...
    for (const [keyvalue] of this.cache.entries()) {
      if (value.expiry <now) {
        this.cache.delete(key);
      }
    }
...
export class SecurityAnalytics {
  ...
  async getSecurityOverview(
    ...
    try {
      ...
      const securityScore = this.calculateSecurityScore({
        ...
        failedLoginRate: authMetrics.failedLogins / Math.max(authMetrics.successfulLogins + authMetrics.failedLogins1),
        eventTrends: {
      ...
      const processedThreatTypes: Array<{ type: string; count: number; percentage: number }> = 
        Object.entries(threatResults.threatTypes || {}).map(([typecount]) => ({
          type,
        ...
        })).sort((ab) => b.count - a.count).slice(05);
  ...
  async getTimelineData(
    ...
    try {
      ...
      const labels = Array.from({ length: intervals }, (_i) => {
      ...
      if (category === 'severity') {
        ...
        datasets = severityLevels.map(severity => {
          const data = Array.from({ length: intervals }, (_i) => {
        ...
        datasets = [
          {
            ...
            data: Array.from({ length: intervals }, (_i) => {
              const intervalStart = startTime + i * intervalMs;
              const intervalEnd = intervalStart + intervalMs;
    ...
    try {
      ...
      switch (category) {
        ...
        case 'threats':
          ...
          const threatsByType = Object.entries(threatResults.threatTypes || {}).map(([typecount]) => ({
            type,
            count: typeof count === 'number' ? count : 0, // Ensure count is a number
            percentage: (typeof count === 'number' ? count : 0) / Math.max(threatResults.threatCount1) * 100
  ...
  private calculateTopEvents(
    ...
    return Object.entries(eventCounts)
      .map(([keycount]) => {
        const [categoryaction] = key.split(':');
  ...
  private calculateGeographicData(
    ...
    return Object.entries(locationCounts)
      ...
      .sort((ab) => b.eventCount - a.eventCount);
  }
