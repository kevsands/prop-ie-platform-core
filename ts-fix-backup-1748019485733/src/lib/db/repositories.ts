...
abstract class BaseRepository<T> {
  ...
  async create(entity: Omit<T, 'id' | 'createdAt' | 'updatedAt'>): Promise<T> {
    try {
      ...
      const placeholders = keys.map((_index) => `$${index + 1}`).join(', ');
  ...
  async update(id: string, entity: Partial<T>): Promise<T> {
    try {
      ...
      const setClause = keys.map((keyindex) => `${key} = $${index + 2}`).join(', ');
...
class UnitRepository extends BaseRepository<Unit> {
  ...
  async findByDevelopment(developmentId: string, filters?: UnitFilter): Promise<Unit[]> {
    ...
    try {
      const units = await unitDb.getByDevelopment(developmentIdfilters);
  ...
  async getCustomizationOptions(unitId: string, categoryFilter?: string): Promise<CustomizationOption[]> {
    ...
    try {
      const options = await unitDb.getCustomizationOptions(unitIdcategoryFilter);
