...
export class ConveyancingService extends EventEmitter {
  ...
  async updateCaseStatus(
    ...
    await this.handleStatusChange(caseIdnewStatus);
  ...
  private async createInitialTasks(
    ...
    return taskTemplates.map((templateindex) => ({
  ...
  async generateDocument(
    ...
    const generatedContent = await this.processTemplate(templateIdvariables);
  ...
  async generateCaseReport(caseId: string): Promise<any> {
    ...
    return {
      ...
      documents: documents,
      financials: fees,
      timeline: this.generateCaseTimeline(caseDatatasks)
    };
  }
  ...
  private generateCaseTimeline(
    ...
    ].sort((ab) => a.date.getTime() - b.date.getTime());
  }
