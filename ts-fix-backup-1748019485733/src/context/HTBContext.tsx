...
export const HTBProvider: React.FC<{ children: ReactNode }> = ({ children }) => {
  const [selectedBuyerClaimsetSelectedBuyerClaim] = useState<HTBClaim | null>(null);
  ...
  const value: HTBContextType = {
    ...
    updateAccessCode: async (id, accessCodeexpiryDate) => {
      try {
    ...
    processAccessCode: async (claimIdaccessCode) => {
      try {
    ...
    addNoteToHTB: async (claimId, contentisPrivate) => {
      try {
    ...
    updateClaimCode: async (claimId, claimCode, claimCodeExpiryDate, approvedAmountdocumentFile) => {
      try {
    ...
    completeHTBClaim: async (claimId, completionDate, notesdocumentFile) => {
      try {
        const formData = new FormData();
    ...
    markDepositApplied: async (claimId, appliedDate, notesdocumentFile) => {
      try {
        const formData = new FormData();
    ...
    uploadHTBDocument: async (claimId, file, typename) => {
      try {
        const formData = new FormData();
    ...
    markFundsReceived: async (claimId, receivedAmount, receivedDatedocumentFile) => {
      try {
        const formData = new FormData();
    ...
    requestClaimFunds: async (claimId, requestDate, notesdocumentFile) => {
      try {
        const formData = new FormData();
